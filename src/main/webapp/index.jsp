<%@page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8"%>
<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <title>Главная страница</title>
    <link rel="stylesheet" href="/css/styles_menu.css">
    <link rel="stylesheet" href="/css/styles_main_page.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&display=swap" rel="stylesheet">
</head>
<body>
<main>
    <div class="heading__bg-image" style="background-image: url('static/images/blur-left-default.png');" bis_skin_checked="1"></div>
<header class="menu">
    <ul class="menu-ul">
        <li class="menu-li"><a class="menu-a" data-text='Главная' href="/">Главная</a></li>
        <li  class="menu-li"><a class="menu-a" data-text="Пользователи" href="/people">Пользователи</a></li>
        <li  class="menu-li"><a class="menu-a" data-text="Добавить" href="/people/add">Добавить</a></li>
        <li  class="menu-li"><a class="menu-a" data-text="Книги" href="/book">Книги</a></li>
    </ul>
</header>
<div class="title-page" bis_skin_checked="1">
    <h1 class="h1">Добро пожаловать!</h1>
</div>
</main>
</body>
</html>